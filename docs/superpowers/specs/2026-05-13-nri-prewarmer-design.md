...
| Event | Action |
|---|---|
| `AddFunc(pod)` | If `pod.DeletionTimestamp == nil`, attempt to acquire a semaphore slot (non-blocking — on saturation, increment `prewarm_error_total{reason="semaphore_full"}` and return). Spawn `go func()` which calls `p.resolveMapping(ctx, pod.UID, pod.Namespace, pod.Name)` and releases the slot. Pods that acquire `DeletionTimestamp` between event dispatch and goroutine start are NOT re-checked here — `DeleteFunc` will evict any cached entry shortly after, and the revoker `safetyNetSync` (5 min) revokes any orphan Vault credentials. Singleflight (`plugin.go`) dedupes if `CreateContainer` raced ahead. Note: `resolveMapping` will internally call `fetchAndBuildMapping` which does the live apiserver GET — the lister `pod.UID` passed here is just an enqueue key, not the trust anchor. |
...
| Unit | `prewarmer.AddFunc` skips terminating pods at handler entry | `kubernetes/fake` + `*corev1.Pod` with `DeletionTimestamp != nil`; assert no fetch |
