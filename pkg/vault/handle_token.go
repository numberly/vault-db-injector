			...
			if _, found := podInfoMap[ki.PodNameUID]; found {
				...
				if err != nil {
					if isLeaseUnrecoverable(err) {
						...
						metrics.FlushPodSeries(ki.PodNameUID, ki.Namespace)
				...
				// Only the series written in this process can be flushed here;
				// the common case (revoker deleted the KV entry first) is
				// covered by the renewer's per-cycle vanished-uuid flush.
				metrics.FlushPodSeries(ki.PodNameUID, ki.Namespace)
